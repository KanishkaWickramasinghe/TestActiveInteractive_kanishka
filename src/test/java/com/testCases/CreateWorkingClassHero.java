...
import com.pages.ClerkDashboard;
...
public class CreateWorkingClassHero extends BaseTest{

	TestUtilities utl= new TestUtilities();
	String id1;
	LoginPage login;
	UploadCSVPage upload;
	ClerkDashboard clerk;
...
//		
		...
		File file=new File(".\\create_Hero.json");
	...
	void createWorkingCLassHerosWithVouchers() throws FileNotFoundException {
		...
		File file=new File(".\\hero_vouchers.json");
		FileReader fR=new FileReader(file);
		JSONTokener jT=new JSONTokener(fR);
		JSONObject requestBody=new JSONObject(jT);
		System.out.println(requestBody);
	...
	public void verifyHerosOwningMoney() throws FileNotFoundException {
		...
		File file=new File(".\\idFile.json");
		FileReader fR=new FileReader(file);
		JSONTokener jT=new JSONTokener(fR);
